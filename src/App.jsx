...
Be their strategic advisor. Analyze their questions through the lens of their specific goals and market. Give data-driven, psychology-based advice. Be direct and specific. Max 4 sentences. Never generic. Always tactical.`,
    ...
    categories: [
      ...
      { id: "learning", label: "Study & Research", icon: "📚", color: "#00CFFF" },
...
Sé su asesor estratégico. Analiza sus preguntas a través de sus metas específicas y mercado. Da consejos basados en datos y psicología. Sé directo y específico. Máximo 4 oraciones. Nunca genérico. Siempre táctico. Responde siempre en español.`,
    ...
    categories: [
      ...
      { id: "learning", label: "Estudio e Investigación", icon: "📚", color: "#00CFFF" },
...
export default function MissionControl() {
  ...
  function renderStrategy(text) {
    if (!text) return null;
    return text.split('\n').map((line, i) => {
      if (line.startsWith('## ')) return <div key={i} style={{ fontSize: 11, letterSpacing: 2, color: "#00CFFF", marginTop: 18, marginBottom: 8, fontWeight: "bold" }}>{line.replace('## ', '▶ ')}</div>;
      ...
      return <div key={i} style={{ fontSize: 12, color: "#C0C8E0", lineHeight: 1.7, marginBottom: 2 }}>{line}</div>;
    });
  }
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#050810", color: "#E0E8FF", fontFamily: "'Courier New', monospace" }}>
      ...
      {notification && (
        <div style={{ position: "fixed", top: 20, right: 20, zIndex: 10000, background: notification.type === "boss" ? "#FF4444" : notification.type === "badge" ? "#FFD700" : "#00FF88", color: "#000", padding: "12px 20px", fontWeight: "bold", fontSize: 13, border: "2px solid currentColor", boxShadow: "0 0 20px currentColor" }}>
          {notification.msg}
        </div>
      )}
      ...
      <style>{`
        ...
        .tab-btn{background:transparent;border:1px solid #1a2040;color:#6080AA;padding:8px 10px;cursor:pointer;font-family:'Courier New',monospace;font-size:9px;letter-spacing:1px;transition:all 0.2s;text-transform:uppercase}
        ...
        .gbtn{background:#FFD700;color:#000;border:none;padding:10px 20px;font-family:'Courier New',monospace;font-weight:bold;cursor:pointer;letter-spacing:1px;font-size:11px;transition:all 0.2s;text-transform:uppercase}
        .gbtn:hover{background:#FFE44D;box-shadow:0 0 14px #FFD70077}
        .gbtn:disabled{background:#1a2040;color:#4a5060;cursor:not-allowed;box-shadow:none}
        ...
        .lbrow{display:flex;align-items:center;gap:12px;padding:12px 14px;border:1px solid #1a2040;background:#080d1a;margin-bottom:7px}
      ...
      <div style={{ maxWidth: 860, margin: "0 auto", padding: "18px 14px" }}>
        ...
        <div style={{ textAlign: "center", marginBottom: 20, paddingBottom: 18, borderBottom: "1px solid #1a2040", position: "relative" }}>
          <div style={{ position: "absolute", right: 0, top: 0, display: "flex", gap: 4 }}>
            {["es","en"].map(l => (
              <button key={l} onClick={() => setLang(l)} style={{ background: lang === l ? "#00CFFF22" : "transparent", border: `1px solid ${lang === l ? "#00CFFF" : "#1a2040"}`, color: lang === l ? "#00CFFF" : "#4a6080", padding: "5px 10px", cursor: "pointer", fontFamily: "monospace", fontSize: 10, letterSpacing: 1 }}>{l.toUpperCase()}</button>
            ))}
          </div>
        ...
        <div style={{ marginBottom: 16, background: "#080d1a", border: "1px solid #1a2040", padding: "10px 14px" }}>
          <div style={{ display: "flex", justifyContent: "space-between", fontSize: 9, color: "#4a6080", marginBottom: 6 }}>
        ...
        {activeTab === "dashboard" && (
          <div style={{ animation: "fadeUp 0.3s ease" }}>
            ...
            {!hasStrategy && (
              <div style={{ background: "#080d1a", border: "2px solid #00CFFF44", padding: 22, marginBottom: 14, textAlign: "center" }}>
                <div style={{ fontSize: 32, marginBottom: 10 }}>🧠</div>
                <div style={{ fontSize: 13, color: "#00CFFF", letterSpacing: 2, marginBottom: 8 }}>{lang === "es" ? "SIN ESTRATEGIA AÚN" : "NO STRATEGY YET"}</div>
                <div style={{ fontSize: 11, color: "#6080AA", marginBottom: 14, lineHeight: 1.7 }}>{t.strategy_subtitle}</div>
            ...
            {hasStrategy && state.userProfile && (
              <div style={{ background: "#080d1a", border: "1px solid #00CFFF22", padding: 14, marginBottom: 14 }}>
                <div style={{ fontSize: 9, letterSpacing: 2, color: "#00CFFF", marginBottom: 10 }}>▶ {lang === "es" ? "PERFIL ESTRATÉGICO" : "STRATEGIC PROFILE"}</div>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                  {[
                    ...
                    <div key={item.label} style={{ background: "#0d1525", border: "1px solid #1a2040", padding: "8px 10px" }}>
                      <div style={{ fontSize: 8, color: "#4a6080", letterSpacing: 2, marginBottom: 4 }}>{item.label}</div>
          ...
          <div style={{ animation: "fadeUp 0.3s ease" }}>
            ...
            {onboardingStep === 0 && !hasStrategy && (
              <div style={{ textAlign: "center", padding: "36px 20px" }}>
                ...
                <div style={{ fontSize: 14, letterSpacing: 3, color: "#00CFFF", marginBottom: 10 }}>{t.strategy_title}</div>
                <div style={{ fontSize: 12, color: "#6080AA", lineHeight: 1.8, maxWidth: 480, margin: "0 auto 24px" }}>{t.strategy_subtitle}</div>
              ...
              <div style={{ animation: "fadeUp 0.3s ease" }}>
                <div style={{ display: "flex", gap: 4, marginBottom: 20 }}>
                  {t.onboarding.map((_, i) => (
                    <div key={i} style={{ flex: 1, height: 3, background: i < onboardingStep ? "#00CFFF" : "#1a2040", transition: "background 0.3s", borderRadius: 2 }} />
                  ))}
                </div>
                ...
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                  ...
                  {onboardingStep > 1 && <button onClick={() => { setOnboardingStep(s => s - 1); setCurrentAnswer(""); }} style={{ background: "transparent", border: "1px solid #1a2040", color: "#4a6080", padding: "10px 16px", cursor: "pointer", fontFamily: "monospace", fontSize: 10 }}>{lang === "es" ? "← ATRÁS" : "← BACK"}</button>}
                  <button onClick={nextOnboardingStep} style={{ background: "transparent", border: "1px solid #1a2040", color: "#4a6080", padding: "10px 16px", cursor: "pointer", fontFamily: "monospace", fontSize: 10 }}>{lang === "es" ? "OMITIR" : "SKIP"}</button>
                </div>
              </div>
            )}
            ...
            {onboardingStep === 8 && (
              <div style={{ textAlign: "center", padding: "56px 20px" }}>
                ...
                <div style={{ fontSize: 12, letterSpacing: 3, color: "#00CFFF", marginBottom: 8 }}>{t.analyzing}</div>
                <div style={{ fontSize: 10, color: "#4a6080", animation: "pulse 1.5s infinite" }}>{t.analyzing_sub}</div>
              </div>
            )}
            ...
            {(hasStrategy && (onboardingStep === 0 || onboardingStep === 9)) && (
              <div style={{ animation: "fadeUp 0.3s ease" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
                  <div style={{ fontSize: 9, letterSpacing: 2, color: "#4a6080" }}>{t.your_strategy}</div>
                ...
                <div style={{ background: "#080d1a", border: "1px solid #00CFFF22", padding: 20, marginBottom: 16, maxHeight: 500, overflowY: "auto" }}>
                ...
                {buildingSchedule && (
                  <div style={{ textAlign: "center", marginTop: 16, color: "#4a6080", fontSize: 11, animation: "pulse 1.5s infinite" }}>{t.building_sub}</div>
                )}
              </div>
            )}
          </div>
          ...
          <div style={{ animation: "fadeUp 0.3s ease" }}>
            {!hasSchedule && (
              <div style={{ textAlign: "center", padding: "40px 20px" }}>
                ...
                <div style={{ fontSize: 12, color: "#6080AA", marginBottom: 18, lineHeight: 1.7 }}>{t.no_schedule}</div>
                  ...
                  return (
                    <div key={idx} className={`trow ${task.completed ? "done" : ""}`} onClick={() => !task.completed && completeScheduleTask(DAYS[selectedDay], idx)}>
                      ...
                      <div style={{ flex: 1 }}>
                        ...
                        <div style={{ fontSize: 10, color: "#4a6080", marginTop: 2, display: "flex", gap: 10, flexWrap: "wrap" }}>
                          <span style={{ color }}>{icon} {task.category}</span>
                          <span>⏱ {task.duration}</span>
                          <span style={{ color: "#FFD700" }}>+{task.xp} XP</span>
                        </div>
                        {task.why && <div style={{ fontSize: 10, color: "#4a6080", marginTop: 3, fontStyle: "italic" }}>💡 {task.why}</div>}
                      </div>
                    </div>
                  );
                })}
        ...
        {activeTab === "tasks" && (
          <div style={{ animation: "fadeUp 0.3s ease" }}>
            ...
            {showAddTask && (
              <div style={{ background: "#080d1a", border: "1px solid #00CFFF33", padding: 14, marginBottom: 14 }}>
        ...
        {activeTab === "coach" && (
          <div style={{ animation: "fadeUp 0.3s ease" }}>
            ...
            {hasStrategy && state.userProfile && (
              <div style={{ background: "#080d1a", border: "1px solid #00CFFF22", padding: "8px 12px", marginBottom: 12, fontSize: 10, color: "#4a6080" }}>
                🧠 {lang === "es" ? "Coach personalizado para:" : "Personalized coach for:"} <span style={{ color: "#00CFFF" }}>{state.userProfile.niche}</span> · {lang === "es" ? "Meta:" : "Goal:"} <span style={{ color: "#FFD700" }}>{state.userProfile.goal_90}</span>
              </div>
            )}
            <div ref={chatRef} style={{ background: "#080d1a", border: "1px solid #00CFFF22", padding: 14, minHeight: 240, maxHeight: 340, overflowY: "auto", marginBottom: 12 }}>
              ...
              {aiLoading && (
                <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                  ...
                  <div style={{ color: "#00CFFF", fontSize: 10, animation: "pulse 1s infinite" }}>{t.incoming}</div>
                </div>
              )}
            </div>
          ...
          <div style={{ animation: "fadeUp 0.3s ease" }}>
            {!state.playerName && (
              <div style={{ background: "#080d1a", border: "1px solid #00CFFF44", padding: 20, marginBottom: 20, textAlign: "center" }}>
                ...
                <div style={{ fontSize: 11, color: "#6080AA", marginBottom: 16, lineHeight: 1.7 }}>
            ...
            {state.playerName && (
              <div style={{ background: "#080d1a", border: "1px solid #00CFFF22", padding: "10px 14px", marginBottom: 14, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            ...
            {leaderboardLoading && <div style={{ textAlign: "center", padding: 24, color: "#4a6080", fontSize: 11, animation: "pulse 1s infinite" }}>{lang === "es" ? "Cargando..." : "Loading..."}</div>}
              ...
              <>
                ...
                {leaderboard.map((player, i) => (
                  <div key={player.name} className={`lbrow ${player.name === state.playerName ? "me" : ""} ${i === 0 ? "first" : ""}`}>
                    <div style={{ fontSize: 13, fontWeight: "bold", color: i === 0 ? "#FFD700" : i === 1 ? "#C0C0C0" : i === 2 ? "#CD7F32" : "#4a6080", minWidth: 24, textAlign: "center" }}>
                      {i === 0 ? "🥇" : i === 1 ? "🥈" : i === 2 ? "🥉" : `#${i+1}`}
                    </div>
                    <div style={{ flex: 1 }}>
              ...
              <>
                ...
                {allTimeBoard.map((player, i) => (
                  <div key={player.name} className={`lbrow ${player.name === state.playerName ? "me" : ""}`}>
                    <div style={{ fontSize: 13, fontWeight: "bold", color: i === 0 ? "#AA44FF" : "#4a6080", minWidth: 24, textAlign: "center" }}>{i === 0 ? "👑" : `#${i+1}`}</div>
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: 13, color: player.name === state.playerName ? "#00CFFF" : "#E0E8FF" }}>
                        {player.name} {player.name === state.playerName && <span style={{ fontSize: 9, color: "#00CFFF" }}>{t.you_label}</span>}
                      </div>
